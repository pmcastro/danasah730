...
void calcula_atacantes(int casilla) {
	int i, n, ca_orig;
	register int ndir;
	int numB = num_atacantes[BLANCO], numN = num_atacantes[NEGRO];
...
int see(int turno2, int casilla, int de) {
	int i, n, d;
	...
	/* salida rapida */
	if (valor_pieza2[pieza[casilla]] >= valor_pieza2[pieza[de]]) return 0;
	...
	num_atacantes[BLANCO] = 0;
	num_atacantes[NEGRO] = 0;
	d = 0;
	...
	/* inicialmente ganamos la pieza que capturamos */
	valorsee[d] = valor_pieza2[pieza[casilla]];
	...
	while(num_atacantes[turno2])
	{
		...
		valorsee[d] = -valorsee[d-1] + ganancia;
...
int see2(int turno2, int casilla, int de) {
	int i, n, d;
	int valor_del_menor, indice_del_menor, casilla_del_menor, valor_temp;
	int valorsee[32];
	int origpieza, origcolor;
	...
	while(num_atacantes[turno2])
	{
		...
		if (-MIN(-valorsee[d], valorsee[d-1]) < 0) break;
