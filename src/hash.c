...
void store_hash(char type, int depth, int score, movimiento hmov) {
    ...
    TThash->mov_de = (char)hmov.de;
    TThash->mov_a = (char)hmov.a;
    TThash->mov_tipo = (char)hmov.tipo;
    TThash->Hash = hash;
    TThash->Type = type;
...
int probe_hash(char *type, int depth, int beta, movimiento *hmov, int *hacer_nulo) {
	U64 index = hash & (TTSize-1);
	TType *TThash;
	int score, ttdepth, i;
	movimiento mov;
    ...
    *hacer_nulo = VERDADERO;
	...
	for (i=0; i<CLUSTER_SIZE; i++) {
        ...
        if (TThash->Hash == hash) {
            ...
            mov.valor = 0;
            *hmov = mov;
            ttdepth = (int)TThash->Depth;
            /* comprobamos si debemos evitar hacer el movimiento nulo */
            if (ttdepth >= depthnull+njugadas && TThash->Value < beta && TThash->Type == hasfALPHA)
                *hacer_nulo = FALSO;
            /* solo devolvemos un resultado si la profundidad de las tablas es mayor o igual que la actual */
            if (ttdepth >= depth) {
                *type = TThash->Type;
                score = (int)TThash->Value;
